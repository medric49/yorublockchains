...
const generateNextBlock = (productId: number) => {
    const coinbaseTx: Transaction = getCoinbaseTransaction(getPublicFromWallet(), getLatestBlock().index + 1, productId);
    const blockData: Transaction[] = [coinbaseTx];
    return generateRawNextBlock(blockData);
};

const generatenextBlockWithTransaction = (receiverAddress: string, productId: number) => {
    ...
    const tx: Transaction = createTransaction(receiverAddress, productId, getPrivateFromWallet(), unspentTxOuts);
...
const getAccountProduct = (): number[] => {
...
export {
    ...
    getAccountProduct, isValidBlockStructure, replaceChain, addBlockToChain
};
