...
class UnspentTxOut {
    ...
    constructor(txOutId: string, txOutIndex: number, address: string, productId: number) {
...
class TxOut {
    ...
    constructor(address: string, productId: number) {
        this.address = address;
        this.productId = productId;
    }
}
...
const getTransactionId = (transaction: Transaction): string => {
    ...
    const txOutContent: string = transaction.txOuts
        .map((txOut: TxOut) => txOut.address + txOut.productId)
        .reduce((a, b) => a + b, '');
...
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
    ...
    if (transaction.txOuts.length !== 1 || transaction.txIns.length !== 1) {
        console.log('There are more than one txOut or more than one  txIns');
        return false;
    }
    const txInProductId: number = transaction.txIns
        .map((txIn) => getTxInProductId(txIn, aUnspentTxOuts))[0];

    const txOutProductId: number = transaction.txOuts
        .map((txOut) => txOut.productId)[0];
    if (txInProductId !== txOutProductId) {
        console.log('txInProductId !== txInProductId in tx: ' + transaction.id);
        return false;
    }
...
const validateBlockTransactions = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[], blockIndex: number): boolean => {
    ...
    if (!validateCoinbaseTx(coinbaseTx, blockIndex) && !validateTransaction(coinbaseTx, aUnspentTxOuts)) {
        console.log('invalid transaction: ' + JSON.stringify(coinbaseTx));
        return false;
    }
...
const getTxInProductId = (txIn: TxIn, aUnspentTxOuts: UnspentTxOut[]): number => {
    return findUnspentTxOut(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts).productId;
};
...
const getCoinbaseTransaction = (address: string, blockIndex: number, productId: number): Transaction => {
    ...
    t.txIns = [txIn];
    t.txOuts = [new TxOut(address, productId)];
...
const updateUnspentTxOuts = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut[] => {
    const newUnspentTxOuts: UnspentTxOut[] = aTransactions
        .map((t) => {
            return t.txOuts.map((txOut, index) => new UnspentTxOut(t.id, index, txOut.address, txOut.productId));
        })
...
const isValidTxOutStructure = (txOut: TxOut): boolean => {
    if (txOut == null) {
        ...
        return false;
    } else if (typeof txOut.productId !== 'number') {
        console.log('invalid product id type in txOut');
        return false;
    } else {
        return true;
    }
